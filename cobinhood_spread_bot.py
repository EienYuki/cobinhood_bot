...
class bot_spread_cobinhood():
	def __init__(self, env):
		...
		# 進行交易的資金百分比
		# 說明: 會依照你的 USDT餘額 x investment_ratio 來進行交易 (請確保金額大於最低交易門檻)
		self.conf.investment_ratio = 0.15

		# 價差百分比
		# 說明: 這會影響掛單的價格 大小可以參考 交易所的深度圖來設定
		self.conf.spread_ratio = 0.5

		# 停損買
		# 說明: 高於這個價格 就會下 市場買單 之後不做任何事
		self.conf.stop_loss_bid = 6800

		# 停損賣
		# 說明: 低於這個價格 就會下 市場賣單 之後不做任何事
		...
		### 以下參數是 趨勢的判斷 請自行調整後 編寫相都應的掛單做法 預設都是一樣的 ###
